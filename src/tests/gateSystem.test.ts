...
describe('Gate System Tests', () => {
  ...
  describe('Gate Diagnostics', () => {
    it('should generate appropriate diagnostics for lint issues', () => {
      ...
      expect(result.diagnostics.some((d: { gate: string }) => d.gate === 'lint')).toBe(true);
      expect(result.diagnostics.some((d: { severity: string }) => d.severity === 'warning')).toBe(true);
    });
  ...
  // @ts-expect-error - intentionally invalid structure for testing
      ...
      expect(result.diagnostics.length).toBeGreaterThan(0);
      expect(result.diagnostics.some((d: { gate: string }) => d.gate === 'types')).toBe(true);
      expect(result.diagnostics.some((d: { severity: string }) => d.severity === 'error')).toBe(true);
    });
    ...
    it('should aggregate diagnostics from all gates', async () => {
      ...
      const gateTypes = [...new Set(result.diagnostics.map((d: { gate: string }) => d.gate))];
      expect(gateTypes.length).toBeGreaterThan(1);
      
      // Should have both errors and warnings
      const severities = [...new Set(result.diagnostics.map((d: { severity: string }) => d.severity))];
