...
import { createEmptyGraph, createComponent, Command } from '../schema';
...
describe('Agent System', () => {
  ...
  describe('Patcher Module', () => {
    ...
    it('should handle errors gracefully', () => {
      ...
      plan.operations[0] = { ...plan.operations[0], id: '' } as unknown as Command;
  ...
  describe('Validator Module', () => {
    ...
    it('should detect duplicate IDs', async () => {
      ...
      expect(result.diagnostics.some((d: { message: string }) => d.message.includes('Duplicate'))).toBe(true);
    });
    ...
    it('should detect out of bounds components', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.diagnostics.some((d: { message: string }) => d.message.includes('out of bounds'))).toBe(true);
    });
  });
